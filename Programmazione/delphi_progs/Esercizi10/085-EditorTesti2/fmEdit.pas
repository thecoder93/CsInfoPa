...
type
  ...
  public
    ...
    procedure CaricaFile(const NomeFile: string );
  end;
...
begin
  if not Conferma then // prima controllo se c'è da confermare
    exit; // eventualmente esco e non faccio nulla
  if odApri.Execute then// se l'utente ha scelto un file
    CaricaFile( odApri.FileName );
end;
...
begin
  // uso una funzione per impostare la variabile interna...
  NomeFile := value;
  // ...la Caption della form
  Caption := ExtractFileName( value ) + ' - Semplice editor';
  // ...e il titolo dell'applicazione
...
procedure TfmEditor.CaricaFile(const NomeFile: string);
begin
  etTesto.Lines.LoadFromFile( NomeFile ); // lo carico nel Memo
  SetNomeFile( NomeFile ); // imposto il nome
...
var
  ...
  fn: string;
begin
// controllo quanti file ci sono da caricare...
  NumFiles := DragQueryFile( Msg.Drop, $FFFFFFFF, nil, 0);
  if NumFiles > 0 then begin
// se ce ne sono diversi prendo solo il primo
    DragQueryFile( Msg.Drop, 0, @buffer, sizeof(buffer));
// metto il nome in una stringa
    fn := buffer;
    CaricaFile( fn );
    end;
// chiudo l'operazione
